...
function startIndexer(io) {
    ...
    console.log(`[Indexer] Marketplace Contract Address: ${exports.MARKETPLACE_ADDRESS}`);
    try {
        const provider = new ethers_1.ethers.JsonRpcProvider(rpcUrl);
        const nftContract = new ethers_1.ethers.Contract(exports.PUFF_NFT_ADDRESS, PUFF_NFT_ABI, provider);
        const marketplaceContract = new ethers_1.ethers.Contract(exports.MARKETPLACE_ADDRESS, MARKETPLACE_ABI, provider);
        ...
        nftContract.on("Transfer", async (from, to, tokenId, event) => {
            try {
                ...
                let fromUser = await db_1.default.user.findUnique({
                    where: { walletAddress: from.toLowerCase() }
                });
                if (!fromUser) {
                    fromUser = await db_1.default.user.create({
                ...
                await db_1.default.transfer.upsert({
                    where: { txHash },
                    update: {},
                    create: {
                        tokenId: dbTokenId,
        ...
        marketplaceContract.on("ItemListed", async (sellerAddress, nftAddress, tokenId, priceInWei, paymentToken, event) => {
            try {
                const tokenStr = tokenId.toString();
                let decimals = 18;
                if (paymentToken && paymentToken !== ethers_1.ethers.ZeroAddress) {
                    try {
                        const tokenContract = new ethers_1.ethers.Contract(paymentToken, ["function decimals() view returns (uint8)"], provider);
                ...
                const priceInPuff = ethers_1.ethers.formatUnits(priceInWei, decimals);
                const txHash = event?.log?.transactionHash || event?.transactionHash || "";
                const blockNumber = event?.log?.blockNumber || event?.blockNumber || 0;
                let timestamp = new Date();
                try {
                ...
                let seller = await db_1.default.user.findUnique({
                    where: { walletAddress: sellerAddress.toLowerCase() }
                });
                if (!seller) {
                    seller = await db_1.default.user.create({
                ...
                if (activeListing) {
                    activeListing = await db_1.default.listing.update({
                ...
                await db_1.default.priceHistory.create({
                    data: {
        ...
        marketplaceContract.on("ItemBought", async (buyerAddress, nftAddress, tokenId, priceInWei, paymentToken, event) => {
            try {
                const tokenStr = tokenId.toString();
                let decimals = 18;
                if (paymentToken && paymentToken !== ethers_1.ethers.ZeroAddress) {
                ...
                let buyer = await db_1.default.user.findUnique({
                    where: { walletAddress: buyerAddress.toLowerCase() }
                });
                if (!buyer) {
                    buyer = await db_1.default.user.create({
                ...
                if (activeListing) {
                    await db_1.default.listing.update({
                        where: { id: activeListing.id },
                        data: {
                            status: "SOLD",
                            soldAt: timestamp,
