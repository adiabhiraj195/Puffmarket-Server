...
// Load deployments
const tokenDeployment = JSON.parse(fs_1.default.readFileSync("/Users/adityaraj/Documents/Web3 Stuff/contract-puff-market/deployments/localhost/PUFFTOKEN.json", "utf-8"));
const nftDeployment = JSON.parse(fs_1.default.readFileSync("/Users/adityaraj/Documents/Web3 Stuff/contract-puff-market/deployments/localhost/PuffNFT.json", "utf-8"));
const marketplaceDeployment = JSON.parse(fs_1.default.readFileSync("/Users/adityaraj/Documents/Web3 Stuff/contract-puff-market/deployments/localhost/NftMarketplace.json", "utf-8"));
...
async function main() {
    ...
    const wallet0 = new ethers_1.ethers.Wallet(key0, provider);
    // Hardhat Account #1
    const key1 = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d";
    const wallet1 = new ethers_1.ethers.Wallet(key1, provider);
    console.log("----------------------------------------------------------------");
    ...
    // 2. Faucet Claims
    const tokenContract0 = new ethers_1.ethers.Contract(PUFF_TOKEN_ADDRESS, tokenDeployment.abi, wallet0);
    const tokenContract1 = new ethers_1.ethers.Contract(PUFF_TOKEN_ADDRESS, tokenDeployment.abi, wallet1);
    ...
    console.log(`Account 0 PUFF Balance: ${ethers_1.ethers.formatUnits(bal0, 18)} PUFF`);
    console.log(`Account 1 PUFF Balance: ${ethers_1.ethers.formatUnits(bal1, 18)} PUFF`);
    ...
    console.log(`Account 0 PUFF Balance after claim stage: ${ethers_1.ethers.formatUnits(bal0, 18)} PUFF`);
    console.log(`Account 1 PUFF Balance after claim stage: ${ethers_1.ethers.formatUnits(bal1, 18)} PUFF`);
    // 3. Mint NFT (Account 0)
    const nftContract0 = new ethers_1.ethers.Contract(PUFF_NFT_ADDRESS, nftDeployment.abi, wallet0);
    ...
    let tokenId = "";
    const iface = new ethers_1.ethers.Interface(nftDeployment.abi);
    ...
    const userNfts = await userNftsRes.json();
    ...
    const listPrice = ethers_1.ethers.parseUnits("1000", 18); // 1000 PUFF
    const marketplace0 = new ethers_1.ethers.Contract(MARKETPLACE_ADDRESS, marketplaceDeployment.abi, wallet0);
    ...
    const listNotifyData = await listNotifyRes.json();
    ...
    const marketplace1 = new ethers_1.ethers.Contract(MARKETPLACE_ADDRESS, marketplaceDeployment.abi, wallet1);
    ...
    const buyApiData = await buyApiRes.json();
    ...
    const nftDetails = await nftDetailsRes.json();
